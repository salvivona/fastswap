...
/*
    | Launch Date     : February 29, 2021  |
    | Reward Duration : 26 Weeks           | 
    | Total Rewards   : 7500               |
    | End Date        : July 16, 2021      |

    | Note : Change starttime on line : 261 |
...
contract LPTokenWrapper is Ownable {
    ...
    IERC20 public COMP_ETH_FLP = IERC20(0x5f53d5231Bbc1b784Ad55e2F3dAb7f073e0bF888); //--|FLP|--
    ...
    function stake(uint256 amount) public {
        ...
        COMP_ETH_FLP.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        COMP_ETH_FLP.safeTransfer(msg.sender, amount);
    }
}
//-----------------------------------------------------------------------
// --------------------| REWARD AMOUNT: 7500 |-----------------------
//-----------------------------------------------------------------------
contract COMPOUND_JUICY_Pool is LPTokenWrapper {
    IERC20 private fast = IERC20(0xC888A0Ab4831A29e6cA432BaBf52E353D23Db3c2);
    uint256 private constant DURATION = 26 weeks;  //-----| Ending |--------

    uint256 private starttime = block.timestamp;       //-----| Friday 5:30 PM UTC |-----
    uint256 private periodFinish = 0;
    uint256 private rewardRate = 0;
    uint256 private lastUpdateTime;
    uint256 private rewardPerTokenStored;
    uint256 private rewardInterval = 48 hours;
    
    mapping(address => uint256) private userRewardPerTokenPaid;
    mapping(address => uint256) private rewards;
    mapping(address => uint256) private lastTimeRewarded;

    event Transfer(address indexed from, address indexed to, uint256 value);

    modifier checkStart(){
        require(block.timestamp >= starttime,"COMPOUND JUICY_Pool not started yet.");
        _;
    }
   ...
   function getReward() public updateReward(_msgSender()) checkStart {
        ...
        if (reward > 0) {
            ...
            fast.safeTransfer(owner(), fee);      //-------| Transfer fee to owner |---------------

            lastTimeRewarded[_msgSender()] = block.timestamp;

            emit Transfer(address(this), msg.sender, rewardMain);
